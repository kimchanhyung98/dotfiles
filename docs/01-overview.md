...
- macOS 중심 환경에 시스템, Claude, Codex, Antigravity, Hermes, CodeGraph 스택을 통합한다.
...
- **설치와 설정 분리**: 도구의 바이너리 설치는 스크립트가 담당하고, 사용자 설정은 chezmoi가 배포하는 설정 파일(`.tmpl`)이 담당한다. 설치 방식이 바뀌어도 설정은 그대로 유지되고, 설정을 변경해도
  재설치가 필요 없다.
- **설치 경계**: AI core와 Claude/Codex 확장 단계를 분리한다. 다만 현재 core script 안의 일부 installer 실패는 warning으로 계속 진행하므로, 모든 도구가 서로 완전히 격리된 것은 아니다.
