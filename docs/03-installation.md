...
| 04 | ai-tools         | claude, codex, codegraph, hermes, oh-my-codex, opencode, oh-my-opencode, gemini | 최초 1회             | macOS와 동일한 AI 스택을 최소 구성으로 설치                                     |
...
│   opencode, oh-my-opencode, gemini
│
├─ 05 system-baseline
│   시스템 기초 설정
│
...
AI 도구(Claude Code, Codex, Gemini, Copilot, Hermes, CodeGraph)는 설치 채널 정책에 따라 AI 스크립트에서 각 채널(curl/npm)로 관리하고, Brewfile은 Homebrew 직접 관리
대상만 유지한다.
...
| Hermes   | 공식 | 공식 스크립트 (`curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh \| bash`) | curl     |
