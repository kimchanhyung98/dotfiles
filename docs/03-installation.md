...
| projects-bootstrap | 공개 GitHub clone·Doppler `.env` 준비 | 최초 적용, 배포 후 | `run_once_after_` 템플릿이 clone helper 후 Doppler helper를 순차 호출한다. 같은 내용을 성공적으로 실행한 후에는 일반 apply/update에서 재실행하지 않는다. |
...
Doppler tap이 신뢰되지 않아 Homebrew 폴백이 실패하면 사용자가 `brew trust --formula dopplerhq/cli/doppler`로 신뢰한 뒤 `chezmoi apply`를 다시 실행한다. Doppler는 최초 프로젝트 `.env` 동기화에 필요하므로 설치 검사에서 제외하지 않는다.
