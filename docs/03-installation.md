...
| 04 | ai-tools         | claude, codex, codegraph, oh-my-codex, opencode, oh-my-opencode, gemini | 최초 1회             | macOS와 동일한 AI 스택을 최소 구성으로 설치                                     |
...
│
├─ 10 ai-core
│   Claude Code, Codex CLI, Gemini CLI, Copilot CLI, CodeGraph, superpowers
│   Claude Code(curl 스크립트), Codex·Gemini·Copilot·CodeGraph(npm) 설치, superpowers를 ~/superpowers에 clone
│
├─ 11 ai-claude
...
│
├─ 04 ai-tools
│   claude, codex, codegraph, oh-my-codex, opencode, oh-my-opencode,
│   gemini
│
├─ 05 system-baseline
│   시스템 기초 설정
...
AI CLI(Claude Code, Codex, Gemini, Copilot, CodeGraph)는 공식 설치 채널 정책에 따라 AI 스크립트에서 각 공식 채널(curl/npm)로 관리하고, Brewfile은 Homebrew 직접 관리
대상만 유지한다.
