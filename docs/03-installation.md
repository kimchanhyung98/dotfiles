...
| 03 | brew-packages | regular | Brewfile 기반 패키지 설치·upgrade | Brewfile 변경 시 | `zb init --no-modify-path` 후 `zb bundle install`을 실행한다. zerobrew가 없거나 실패하면 `brew bundle`로 폴백하며, 최종 check 실패는 non-zero다. 자동 cleanup은 하지 않는다. |
