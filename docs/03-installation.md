...
| 03 | brew-packages | regular | Brewfile 기반 패키지 설치·upgrade | Brewfile 변경 시 | `zb init --no-modify-path`와 `zb upgrade` 후 `zb bundle install`을 실행한다. zerobrew가 없거나 실패하면 `brew bundle`로 폴백하며, 최종 check 실패는 non-zero다. 자동 cleanup은 하지 않는다. |
...
Brewfile은 `zb init --no-modify-path`로 chezmoi 관리 셸 파일을 건드리지 않고 초기화한 뒤, 설치된 formula의 버전 교체를 `zb upgrade`로 먼저 처리하고 `zb bundle install -f Brewfile`로 누락 formula를 추가하며, 성공하면 종료한다. `zb bundle install`은 기존 버전의 symlink를 교체하지 못하므로 버전 교체는 `zb upgrade`가 담당한다. zerobrew가 없거나 실패한 경우에만 `brew bundle --file=Brewfile`로 폴백한다. 이미 설치된 `xcodes`는 Homebrew 폴백 실행에서만 제외해 bottle 없는 release로의 upgrade 실패를 피하고, 나머지 package는 평소처럼 설치·upgrade한다. Homebrew 폴백의 사후 검증은 보류 중인 upgrade가 아니라 설치 여부만 확인한다. Brewfile에서 빠진 package 제거는 별도 cleanup 명령 없이는 수행하지 않는다.
