...
| 04  | cmux-settings  | cmux 외부 제어 기본 활성화                            | 설정 변경 시           | `~/.config/cmux/settings.json`을 배포하고 `defaults write com.cmuxterm.app socketControlMode automation`을 적용해 신규 macOS 시스템에서 cmux 외부 자동화 제어를 기본 활성화                                                             |
