...
첫 chezmoi 적용 시 apply가 끝난 뒤 `name`의 GitHub 개인 계정에서 공개·비보관 저장소를 최대 100개 조회하여 `~/Documents/GitHub`에 clone한다. 이 단계는 인증을 요구하지 않고 organization을 조회하지 않는다. `run_once_after_` 접두사로 같은 렌더 결과를 다시 실행하지 않으며, 템플릿 내용이 변경되어 재실행되더라도 기존 Git 저장소는 보존한다. 예약 dotfiles 동기화에는 프로젝트 bootstrap을 별도로 호출하는 로직이 없다.
...
| projects-bootstrap | 공개 GitHub 저장소 clone | 최초 적용, 배포 후 | `run_once_after_` 템플릿이 첫 적용 시 helper를 호출한다. 같은 내용을 성공적으로 실행한 후에는 일반 apply/update에서 재실행하지 않는다. |
