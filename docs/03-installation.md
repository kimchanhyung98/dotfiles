...
| 순서 | 스크립트        | 역할                                                           | 실행 조건 | 상세                                                                                                                                |
|:--:|-------------|--------------------------------------------------------------|-------|-----------------------------------------------------------------------------------------------------------------------------------|
| 10 | ai-core     | Claude Code, Codex CLI, Gemini CLI, Copilot CLI, superpowers | 최초 1회 | 공식 AI CLI 도구 4종 설치 + superpowers를 ~/superpowers에 clone. Claude Code는 공식 설치 스크립트(curl), 나머지 3종은 npm. 확장 환경 구성은 개별 스크립트(11~15)에서 처리 |
...
| 15 | ai-humanizer-ko | Claude humanize-korean 직접 복사, Codex humanize-korean 스킬 래퍼 | 스크립트 변경 시 | `~/im-not-ai` checkout을 clone/pull로 유지하고 Claude 글로벌 agents/commands/skills에 복사. Codex에는 원본 references와 헤더만 교체한 SKILL.md를 배포 |
...
- 코어 설치(10)와 프로바이더별 확장 설치(11~15, 20)를 분리하여 책임 경계를 명확히 유지
...
- humanize-korean은 plugin CLI를 사용하지 않고, 15번 스크립트에서 `~/im-not-ai` checkout을 직접 갱신한 뒤 Claude에는 복사, Codex에는 헤더만 교체한 전용 스킬 래퍼로 배포
...
| 15 | ai-humanizer-ko  | Claude humanize-korean 직접 복사, Codex humanize-korean 스킬 래퍼        | 스크립트 변경 시         | `~/im-not-ai` checkout을 기반으로 Claude 글로벌 자산과 Codex 스킬 래퍼를 설치                  |
...
│
├─ 15 ai-humanizer-ko
│   ~/im-not-ai git checkout → Claude global copy → Codex humanize-korean skill wrapper
│
├─ 20 ai-opencode
...
│   ~/.codex/config.toml
│   ~/.agents/skills/*, ~/.agents/skills/humanize-korean, ~/.copilot/mcp-config.json, ~/.copilot/skills/*
...
im-not-ai의 `humanize-korean`은 `.chezmoiexternal.toml`이 아니라 `run_onchange_15-ai-humanizer-ko.sh.tmpl`에서 관리한다. 기본 방식은 plugin marketplace가 아니라 `~/im-not-ai`에 `epoko77-ai/im-not-ai`를 clone하고 이후 `git pull`로 갱신하는 직접 git 경로다. Claude Code에는 이 checkout의 `.claude/agents`, `.claude/commands`, `.claude/skills/humanize-korean`을 글로벌 `~/.claude/` 경로에 복사하고, Codex에는 `~/.agents/skills/humanize-korean/`에 원본 reference와 헤더만 교체한 `SKILL.md`를 배포한다.
