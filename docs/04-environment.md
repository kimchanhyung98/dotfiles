...
| 영역  | 설정                           | 상세                                                        |
|-----|------------------------------|-----------------------------------------------------------|
| 패키지 | curl, git, vim, zsh, bat, zoxide | 패키지 관리자를 자동 감지(apt-get → dnf → yum)하여 설치. Ghostty는 수동 설치 안내만 출력. 이미 설치된 패키지는 건너뜀 |
| 셸   | 기본 셸 zsh 전환, 히스토리, 키바인딩      | Linux script는 `chsh`를 수행하고, 공통 `~/.config/zsh/00-core.zsh`가 히스토리와 emacs 키바인딩을 설정 |
...
Ghostty는 macOS에서 Brewfile의 Homebrew cask로 설치한다. Linux 스크립트는 표준 저장소 패키지로 설치하지 않고, 미설치 시 공식 Linux 설치 문서 안내만 출력한다.
